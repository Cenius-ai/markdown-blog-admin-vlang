...
export async function ensureDatabase(): Promise<void> {
  ...
  // Push the database schema (idempotent)
  try {
    execSync('npx --no-install prisma db push --skip-generate', {
      stdio: 'pipe',
      env: { ...process.env },
    });
    console.log('[auto-setup] Database schema pushed.');
  } catch (err) {
    console.error('[auto-setup] Failed to push database schema:', err);
    return;
  }
  ...
  try {
    ...
    if (!existingUser) {
      ...
      console.log('[auto-setup] Created demo user: cenius@cenius.ai / cenius');
    }
  } catch (err) {
    console.error('[auto-setup] Failed to seed demo user:', err);
  }
  ...
  try {
    ...
    if (count === 0) {
      ...
      console.log(`[auto-setup] Seeded ${seedPosts.length} posts.`);
    } else {
      console.log(`[auto-setup] Database already has ${count} posts, skipping seed.`);
    }
  } catch (err) {
    console.error('[auto-setup] Failed to seed database:', err);
  }
}
